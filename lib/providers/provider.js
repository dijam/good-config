...
class Provider {
   ...
   */
  loadConfig() {
    return Promise.reject(new Error('No provider has been selected or provider does not implement loadConfig().'));
  }
   ...
   */
  loadConfigSync() {
    return Promise.reject(new Error('No provider has been selected or provider does not implement loadConfigSync().'));
  }
  ...
  get() {
    return new Promise((resolve, reject) => {
      ...
      this.files.forEach(file => filesList.push(this.loadConfig(`${this.path}/${file}`)));

      return Promise.all(filesList).then(data => resolve(data)).catch(err => reject(err));
    });
  }
