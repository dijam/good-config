...
function envify(data) {
  ...
  Object.keys(flattened).forEach(key => {
    const constantCasedKey = upperUnderscore(key);
    if (
      flattened[key] !== undefined &&
      Object.prototype.hasOwnProperty.call(envs, constantCasedKey)
    ) {
      ...
      flattened[key] = typeify(envs[constantCasedKey], flattened[key]);
    }
  });
